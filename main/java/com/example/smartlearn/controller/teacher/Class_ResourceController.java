...
public class Class_ResourceController {
    ...
    @Transactional
    public ResponseEntity<?> getTeacherResources(@RequestParam Long teacherId) {
        try {
            List<ClassResource> resources = resourceService.getTeacherResources(teacherId);
            return ResponseEntity.ok(resources);
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("获取资源失败: " + e.getMessage());
        }
    }
