...
import com.example.smartlearn.dto.response.SubmissionResponse;
...
import com.example.smartlearn.dto.response.QuizTaskResponse;
...
import com.example.smartlearn.dto.response.QuizSubmissionResponse;
...
public class StudentQuizService {
            ...
            if (submission.getStudentAnswers() != null) {
                hasUnmarkedSubjective = submission.getStudentAnswers().stream()
                        .anyMatch(ans -> (ans.getQuestion().getType() == com.example.smartlearn.model.Question.QuestionType.SHORT_ANSWER)
                                && (ans.getScore() == null || ans.getScore().compareTo(java.math.BigDecimal.ZERO) == 0));
            }
    ...
    private Integer getQuestionScore(Quiz quiz, Question question) {
        try {
            // 从数据库中查询题目分数
            Integer score = quizQuestionRepository.findScoreByQuizIdAndQuestionId(quiz.getId(), question.getId());
            if (score == null) {
            ...
            log.error("获取题目分数失败，quizId: {}, questionId: {}", quiz.getId(), question.getId(), e);
            // 如果获取分数失败，返回默认分数
            return 10;
        }
    }
